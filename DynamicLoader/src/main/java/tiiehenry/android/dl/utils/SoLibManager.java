...
package tiiehenry.android.dl.utils;
...
public final class SoLibManager {
        ...
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (zipEntryName.endsWith(".so") && zipEntryName.contains(cpuArchitect)) {
                    ...
                    mSoExecutor.execute(new CopySoTask(context, zipFile, zipEntry, lastModify,nativeLibDir));
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private class CopySoTask implements Runnable {
        ...
        CopySoTask(Context context, ZipFile zipFile, ZipEntry zipEntry, long lastModify, String nativeLibDir) {
            mZipFile = zipFile;
            mContext = context;
            mZipEntry = zipEntry;
        ...
        private void writeSoFile2LibDir() throws IOException {
            ...
            File outFile=new File(mNativeLibDir, mSoFileName);
