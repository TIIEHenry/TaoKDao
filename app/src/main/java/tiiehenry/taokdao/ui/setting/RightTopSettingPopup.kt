...
class RightTopSettingPopup(val main: IMainContext, val dataList: MutableList<IPreference<*>>, val view: View)
    : UIRecyclerPopup<IPreference<*>>(main.activity,
        PreferenceAdapter(main, dataList) { list ->
            RightTopSettingPopup(main, list, view).showAt()
        }, DIRECTION_BOTTOM, R.style.Animation_Popup_RightTop
) {
