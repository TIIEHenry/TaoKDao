...
public class TreeViewAdapter<T extends LayoutItemType, VH extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private static final String KEY_IS_EXPAND = "IS_EXPAND";
    private final List<TreeBinder<T, VH>> viewBinders;
    private final List<TreeNode<T>> displayNodes;
    private int padding = 30;
    private boolean toCollapseChild;
    private List<TreeNode<T>> rootNodeList = new ArrayList<>();
...
//                        Log.e("tree", "onBindViewHolder: " );
                        if (listener != null) {
                            listener.onClick(node, (VH) holder, this);
                        }
                        if (!isExpanded)
                            node.collapse();
                        break;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        holder.itemView.setOnClickListener(v -> {
            ...
            if (listener != null && listener.onClick(selectedNode, (VH) holder, this)) {
                return;
            }
        ...
        holder.itemView.setOnLongClickListener(v -> {
            ...
            TreeNodeListener<T, VH> listener = getBinder(selectedNode).getListener();
            if (listener != null) {
                return listener.onLongClick(selectedNode, (VH) holder, this);
            }
            return false;
        });
        for (TreeBinder<T, VH> viewBinder : viewBinders) {
