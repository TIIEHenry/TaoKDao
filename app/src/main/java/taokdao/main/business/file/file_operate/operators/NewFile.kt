...
class NewFile(val main: IMainContext) : SimpleIFileOperator(
        Properties("newFile", main.context, R.string.file_operators_newfile_label)
) {
    override fun call(main: IMainContext, path: String): Boolean {
        val file = File(path)
        Dialogs.global
                .asInput()
                .title(R.string.file_operators_newfile_dialog_title)
                .inputTypeFile()
                .inputRangeFile()
                ...
                .input("name.suffix",
                    ...
                    if (msg == null) {
                        ...
                        msg = if (newFile.exists()) {
                            ...
                            main.getString(R.string.file_operators_newfile_success)
                        } else
                            main.getString(R.string.file_operators_newfile_failed)
                        dialog.dismiss()
                    }
                    main.send(msg)
                }
