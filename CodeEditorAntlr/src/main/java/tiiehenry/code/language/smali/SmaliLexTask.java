...
//            ReferenceFinder.INSTANCE().visit(item, ctx, tks, false, true);
    ...
    @Override
    public void tokenizeIn(List<Span> tokens, List<BlockLine> lines, List<SymbolPair> pairs, List<Variable> variables, SmaliLexer lexer, int fromIndex, int fromLineOffset, int fromLine, String code) {
        isLabel = false;
        if (item != null) {
            item.reset();
        }
