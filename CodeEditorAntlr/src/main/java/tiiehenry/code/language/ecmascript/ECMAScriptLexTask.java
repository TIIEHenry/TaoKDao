...
import tiiehenry.code.antlr4.ECMAScriptLexer;
...
import static tiiehenry.code.antlr4.ECMAScriptLexer.Class;
import static tiiehenry.code.antlr4.ECMAScriptLexer.Enum;
import static tiiehenry.code.antlr4.ECMAScriptLexer.Package;
import static tiiehenry.code.antlr4.ECMAScriptLexer.Void;
import static tiiehenry.code.antlr4.ECMAScriptLexer.*;

public class ECMAScriptLexTask extends Antlr4LexTask<ECMAScriptLexer> {
    private ECMAScriptParser.ProgramContext program;
    private final ECMAScriptParser parser;

    public ECMAScriptLexTask(Language language) {
        super(language);
        setKeywords(ECMAScriptLexer.VOCABULARY);
        parser = new ECMAScriptParser(null);
    }

    @Override
    public ECMAScriptLexer generateLexer() {
        ECMAScriptLexer lexer=new ECMAScriptLexer(null);
    ...
    @Override
    public void tokenizeIn(List<Span> tokens, List<BlockLine> lines, List<SymbolPair> pairs, List<Variable> variables, ECMAScriptLexer lexer, int fromIndex, int fromLineOffset, int fromLine, String code) {
        program = null;
    ...
    @Override
    protected void parse(ECMAScriptLexer lexer) {
    ...
    public synchronized void format(String input, int width, int curPos, FormatCallback callback) throws Exception {
        ECMAScriptLexer lexer = generateLexer();
        lexer.setInputStream(CharStreams.fromString(input));
        CharSequence newStr = ECMAScriptFormatter.getInstance().format(lexer, getLanguage().indentChar, width);
    ...
    public int createAutoIndent(CharSequence subSequence) {
        ...
        return ECMAScriptFormatter.getInstance().createAutoIndent(lexer);
    }
    /*@Override
    protected int formatInThread(StringBuilder sb, IndentStringBuilder isb, ECMAScriptLexer lexer, CharSequence input, int width, int curPos) {
        int newPos = -1;
        int start = -1;
        while (true) {
