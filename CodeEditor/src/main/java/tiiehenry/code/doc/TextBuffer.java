...
public class TextBuffer implements java.lang.CharSequence {
    ...
    private int findCharOffset(int targetLine, int startLine, int startOffset) {
        ...
        EditorException.assertVerbose(isValid(startOffset),
    ...
    private int findCharOffsetBackward(int targetLine, int startLine, int startOffset) {
        ...
        if (offset >= 0) {
            ...
            EditorException.assertVerbose(false,
    ...
    private void onAdd(int charOffset, int totalChars) {
        ...
        if (s.first < _spans.size()) {
            Span p = _spans.get(s.first);
            p.len += totalChars;
        }
    }
    ...
    public void clearSpans() {
        _spans = new Vector<>();
        _spans.add(new Span(0, 0, 0, length(), ColorScheme.Colorable.TEXT));
    }
     ...
     * like color, font, etc.
     *
     * @param spans A collection of PairInts, where Pair.first is the start
