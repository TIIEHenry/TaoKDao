...
public class YoyoNavigationMethod extends TouchNavigationMethod {
    ...
    public YoyoNavigationMethod(EditorField textField) {
        super(textField);
        DisplayMetrics dm = textField.getContext().getResources().getDisplayMetrics();
        _yoyoSize = (int) TypedValue.applyDimension(2, (float) (textField.setting.yoyoSize * 1.4), dm);
//        super.getCaretBloat().left=0;
        _yoyoCaret = new Yoyo();
        _yoyoStart = new Yoyo();
        _yoyoEnd = new Yoyo();
    }
...
//		Log.e(TAG, "onScroll:up "+e2.getAction()+e1.getAction());
        if (_isCaretHandleTouched) {
            _isShowYoyoCaret = true;
    ...
    private void moveHandle(Yoyo _yoyo, MotionEvent e) {
        ...
        if (newCaretIndex >= 0) {
            //isYoyoTouch解决大量move输入法无法点击的情况
            _textField.moveCaret(newCaretIndex,true);
            ...
            _textField.showMagnifier(e.getX(), e.getY()-_yoyoSize/2);
        }
        ...
        public Yoyo() {
            ...
            ColorScheme.addOnColorChangedListener(new ColorScheme.OnColorChangedListener() {
                ...
                public void onColorChanged() {
                    setHandleColor(Colorable.CARET_BACKGROUND.getColor());
                }
            });
        }
...
//            Log.e("yoyo", "draw: " );
            ...
            canvas.drawArc(new RectF(_anchorX - radius, _anchorY - radius / 2f - YOYO_STRING_RESTING_HEIGHT,
                    _handleX + radius * 2, _handleY + radius / 2f), 51, 78, true, _brush);
